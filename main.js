...
const renderList = (list) => {
  tasksList.innerHTML = list
    .map(
      (task) =>
        `<li> ${task.name} <img src= "./img/delete-button.svg" alt= "Boton para borrar tarea" class= "delete-btn" data-id= ${task.tasksId} </li>`
    )
    .join("");
};
...
const deleteTask = (e) => {
  ...
  //TO-DO borrar la tarea de la lista
  //Necesitamos el id de la tarea que estamos apretando
  const id = +e.target.dataset.id; // El + sirve para pasarlo a number.
  tasks = tasks.filter((task) => task.tasksId !== id);

  //Modificar de localstorage
  saveToLocalStorage(tasks);
